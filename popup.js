...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function renderPlayers() {
      ...
      // convenience: ensure at least 1 row when starting fresh
      if (players.length === 0) {
      ...
      if (isLocked()) {
        setStatus("Players submitted (locked). Counter tab can be opened again any time.");
      } else {
        setStatus("");
      }
    }
    ...
    submitPlayersBtn.addEventListener("click", () => {
      ...
      // reset counter + old results for a new round
      ...
      // ✅ auto-open (or focus) the counter tab
      const counterUrl = chrome.runtime.getURL("counter.html");
      chrome.tabs.query({}, (tabs) => {
        const existing = tabs.find(t => t.url === counterUrl);
        if (existing?.id) {
          chrome.tabs.update(existing.id, { active: true });
        } else {
          chrome.tabs.create({ url: counterUrl });
        }
      });
