...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // If all guesses are over, outcome is "none".
    ...
    function init() {
      if (!isLocked()) {
        setStatus("Players not submitted yet. Go back to the Add Players popup first.");
      } else {
        setStatus("");
      }
      ...
      // If a total was already submitted, show the saved result
      const saved = localStorage.getItem(STORAGE_KEYS.winner);
      if (saved) {
        try {
          const outcome = JSON.parse(saved);
    ...
    submitTotalBtn.addEventListener("click", () => {
      ...
      if (!players.length) return setStatus("No players found. Go add players first.");
